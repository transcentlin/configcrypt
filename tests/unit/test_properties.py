...
def valid_password(draw):
    ...
    return draw(
        st.text(
            min_size=8,
            max_size=64,
            alphabet=st.characters(min_codepoint=33, max_codepoint=126),  # 可打印ASCII字符
        )
    )
...
class TestProperty2_OutputFileNaming:
    ...
    def test_encrypt_adds_enc_extension(self, filename, extension):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            encrypted_file = vault.encrypt_file(
                input_file, password="testpass123", delete_source=False
            )
    ...
    def test_decrypt_removes_enc_extension(self, filename):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            encrypted_file = vault.encrypt_file(input_file, password="testpass123")

            # 解密（使用默认输出路径）
            decrypted_file = vault.decrypt_file(encrypted_file, password="testpass123")
...
class TestProperty3_SourceFileDeletionBehavior:
    ...
    @pytest.mark.property
    @given(content=st.binary(min_size=100, max_size=10240), delete_source=st.booleans())
    ...
    def test_delete_source_parameter(self, content, delete_source):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # 加密
            vault.encrypt_file(input_file, password="testpass123", delete_source=delete_source)
...
class TestProperty4_CustomOutputPath:
    ...
    def test_custom_output_path_encrypt(self, content, custom_name):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            encrypted_file = vault.encrypt_file(
                input_file, output_path=custom_output, password="testpass123", delete_source=False
            )
    ...
    def test_custom_output_path_decrypt(self, content, custom_name):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            decrypted_file = vault.decrypt_file(
                encrypted_file, output_path=custom_output, password="testpass123"
            )
