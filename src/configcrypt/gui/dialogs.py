...
def create_dialog_icon(size: int = 32) -> QIcon:
    ...
    painter.drawRoundedRect(
        lock_body_x, lock_body_y, lock_body_width, lock_body_height, size * 0.05, size * 0.05
    )
...
class WelcomeWizard(QDialog):
    ...
    def _on_ok_clicked(self):
        ...
        try:
            ...
            QMessageBox.critical(self, "保存失败", f"无法保存密码到系统密钥链：\n{e}")
...
class EncryptDialog(QDialog):
    ...
    def get_options(self):
        ...
        Returns:
            dict: 包含删除源文件选项
        """
        return {"delete_source": self.delete_source_checkbox.isChecked()}
...
class DecryptDialog(QDialog):
    ...
    def get_options(self):
        ...
        Returns:
            dict: 包含打开编辑器选项
        """
        return {"open_editor": self.open_editor_checkbox.isChecked()}
