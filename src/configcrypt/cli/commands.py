...
def init():
    ...
    # 输入新密码
    while True:
        password = click.prompt("请输入主密码", hide_input=True, confirmation_prompt=True)
...
@click.option("--out", type=click.Path(path_type=Path), help="输出文件路径（默认: 原文件名.enc）")
@click.option("--keep", is_flag=True, help="保留源文件（默认会删除）")
def encrypt(file, out, keep):
    ...
    try:
        ...
        encrypted_path = vault.encrypt_file(file, output_path=out, delete_source=delete_source)
...
@cli.command()
@click.argument("file", type=click.Path(exists=True, path_type=Path))
@click.option("--out", type=click.Path(path_type=Path), help="输出文件路径（默认: 移除.enc扩展名）")
@click.option("--open", "open_editor", is_flag=True, help="解密后立即用编辑器打开文件")
@click.option("--editor", type=str, help="指定编辑器命令（配合--open使用）")
def decrypt(file, out, open_editor, editor):
    ...
    try:
        ...
        # 解密文件
        decrypted_path = vault.decrypt_file(file, output_path=out)
...
def reset_password():
    ...
    click.echo("请设置新的主密码")
    while True:
        new_password = click.prompt("新主密码", hide_input=True, confirmation_prompt=True)
